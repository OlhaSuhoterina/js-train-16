...
function filterStudentsByGrade(students, grade) {
  ...
  let studentFilter = students.filter(function (student) {
...
function displayGroupedInfo(students) {
  for (const value of students) {
    console.group(`Студент: ${value.name}`);
    console.info(`Вік: ${value.age}`);
    console.info(`Предмет: ${value.subject}`);
    console.groupEnd();
  }
...
function countVowelsAndConsonants(word) {
  ...
  for (let x = 0; x < word.length; x++) {
    if (str.indexOf(word[x]) !== -1) {
      vcount += 1;
      console.log(`Голосна:`, vcount);
    } else {
      count += 1;
      console.log(`Приголосна:`, count);
    }
  }
